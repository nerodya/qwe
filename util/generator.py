...
def generate_random_message(num_subscribers: int, max_blocks_per_subscriber: int, max_parameters_per_block: int):
    ...
    for i in range(num_subscribers):
        ...
        max_blocks_per_subscriber = len(number_block_ch) if i % 2 == 0 else len(number_block_noch)
        for j in range(1, max_blocks_per_subscriber):
            function_block = FunctionBlock(
                number_block_ch[j] if i % 2 == 0 else number_block_noch[j])  # Генерация случайного номера блока функций
            for k in range(1, max_parameters_per_block):
                control_parameter = ControlParameter(random.randint(1, 100),
                                                     ...
                                                     max_value=random.randint(71, 100))  # Генерация случайных значений параметров
...
if __name__ == '__main__':
    with connect("ws://localhost:8080") as websocket:
        while True:
            time.sleep(1)
            data = generate_random_message(2, 42, 12)
